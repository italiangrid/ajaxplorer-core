 ...
 */
class lfcAccessWrapper implements AjxpWrapper {
			...
			//print($streamType.$path);
		   	if($streamType == "file"){	
		   		if(self::$crtZip == null ||  !is_array(self::$currentListingKeys)){
		   			...
		   			register_shutdown_function(array("lfcAccessWrapper", "removeTmpFile"), $tmpDir, $tmpFileName);
    ...
    public function stream_stat(){
    	...
    	if(is_resource($this->fp)){
            ...
            lfcAccessWrapper::$lastRealSize = false;
    		if($statValue[2] > 0 && $PROBE_REAL_SIZE && !ini_get("safe_mode")){
	    		lfcAccessWrapper::$lastRealSize = floatval(trim($this->getTrueSizeOnFileSystem($this->realPath)));
    		}
	    	return $statValue;    		
    	}
...
error_log("sono a rename dentro il wrapper");

 //       exec("sudo X509_USER_PROXY=/tmp/x509up_u500 LFC_HOST=lfcserver.cnaf.infn.it -u mbenci lfc-rename $from $to");
//    	return rename($this->initPath($from, "file", false, true), $this->initPath($to, "file", false, true));
	...
	protected function getTrueSizeOnFileSystem($file) {
		if (!(strtoupper(substr(PHP_OS, 0, 3)) == 'WIN')){
			...
			$lfcobj = new COM("Scripting.FileSystemObject");
			$f = $lfcobj->GetFile($file);
