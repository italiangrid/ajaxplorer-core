...
private $metaStore;
	...
	$user_dir="/var/www/html/ajp/data/personal/".$_SESSION['user_id'];
	if(is_dir($user_dir)){
		error_log("esiste la dir");
	} else { 
...
//	error_log("command2 is $command2");
	...
	function switchAction($action, $httpVars, $fileVars){
		...
		case "download";
			...
			if(($_REQUEST["cloud"])=='true'|| ($_REQUEST["big"])=='true'){
			...
			if(($_REQUEST["cloud"])=='true'){
				...
				$con = mysql_connect('flyback.cnaf.infn.it', 'root', 'P0rt@l3');					
					if (!$con)
					  {
					  die('Could not connect: ' . mysql_error());
					  }
				
				mysql_select_db("liferay", $con);			
				$id=$_SESSION['user_id'];
				...
				$user_home_dir_cloud_transfer=$user_home_dir.$random;
				$user_home_dir_cloud_transfer_list=$user_home_dir_cloud_transfer.'/list';
				$user_home_dir_cloud_transfer_finished=$user_home_dir_cloud_transfer.'/ok';
				$user_home_dir_cloud_transfer_error=$user_home_dir_cloud_transfer.'/error';
				$user_home_dir_cloud_transfer_grid_error=$user_home_dir_cloud_transfer.'/grid_error';
	
				mkdir("$user_home_dir_cloud_transfer", 0777);
				touch("$user_home_dir_cloud_transfer_list");
				touch("$user_home_dir_cloud_transfer_finished");
				touch("$user_home_dir_cloud_transfer_error");
				touch("$user_home_dir_cloud_transfer_grid_error");
			
			$numElements=0;
				$str2=array();
				foreach ($selection->getFiles() as $selectedFile){
				$numElements=$numElements+1;
				if (($fd = fopen($user_home_dir_cloud_transfer_list, 'a')) !== false){ 
			...
			foreach ($selection->getFiles() as $selectedFile){  
				...
				exec("php /var/www/html/ajp/plugins/access.lfc/retrieve.php '$selectedFile' '$X509_USER_PROXY' '$lfc_server' '$home' '$protocol' '$server' '$username' '$password' '$random' '$LCG_GFAL_INFOSYS' '$user_home_dir_cloud_transfer' '$numElements' '$mail' '$firstName' '$lastName' '$user_id' '$user_home_dir_cloud_transfer_list' '$vo_active' '$folder' '$port' '$mycloud'> /dev/null &");
			...
			$firstName = $info['firstName'];
			$mail = $info['emailAddress'];					
			$user_home_dir_big_transfer=$user_home_dir.$random;
			$user_home_dir_big_transfer_list=$user_home_dir_big_transfer.'/list';
			$user_home_dir_big_transfer_finished=$user_home_dir_big_transfer.'/ok';
			$user_home_dir_big_transfer_error=$user_home_dir_big_transfer.'/error';
			$user_home_dir_big_transfer_grid_error=$user_home_dir_big_transfer.'/grid_error';

			mkdir("$user_home_dir_big_transfer", 0777);
			touch("$user_home_dir_big_transfer_list");
			touch("$user_home_dir_big_transfer_finished");
			touch("$user_home_dir_big_transfer_error");
			touch("$user_home_dir_big_transfer_grid_error");
			
			$numElements=0;
			$str2=array();
			foreach ($selection->getFiles() as $selectedFile){
			$numElements=$numElements+1;
			if (($fd = fopen($user_home_dir_big_transfer_list, 'a')) !== false){ 
					$str2[$i] = substr($selectedFile, 1);
			    	fwrite($fd, $str2[$i].PHP_EOL);
			    	$c=$selectedFiles[$i];
			    	fclose($fd);
 			 ...
 			 foreach ($selection->getFiles() as $selectedFile){   
			exec("php /var/www/html/ajp/plugins/access.lfc/retrieve.php '$selectedFile' '$X509_USER_PROXY' '$lfc_server' '$home' '$protocol' '$server' '$username' '$password' '$random' '$LCG_GFAL_INFOSYS' '$user_home_dir_big_transfer' '$numElements' '$mail' '$firstName' '$lastName' '$user_id' '$user_home_dir_big_transfer_list' '$vo_active' '$folder' '$port' '$mycloud'> /dev/null &");
   			}			
		} 
...
error_log("sono arrivato7");

			if (!isset($_REQUEST["local"]) || $_REQUEST["local"]!="true") { 	
			
	error_log("sono arrivato0");

				$message = $this->download($selection->getFiles(), $logMessages, $_REQUEST);
			...
			error_log("sono arrivato1");
				...
				foreach ($selection->getFiles() as $selectedFile) {
	                ...
	                $fs_file="/var/www/html/ajp/data/personal/".$_SESSION['user_id']."/".$file_name;
	...
	//          Make a temp zip and send it as download
		...
		//			$file = AJXP_Utils::getAjxpTmpDir()."/".($loggedUser?$loggedUser->getId():"shared")."_".time()."tmpDownload.tar";
					$file='/var/www/html/ajp/data/personal/'.$this->user_id.'/tmpDownload.tar';
	...
	//			$zipFile = $this->makeZip($file_names, $file, $dir);
				...
				if ($keep!="true" && $request["big"]!='true') {
					foreach($file_names as $file_to_delete) {
							unlink('/var/www/html/ajp/data/personal/'.$this->user_id.'/'.$file_to_delete);
							error_log("file da cancellare $file_to_delete");
					}
				}
						
	            } else {
		            
		            error_log("sono arrivato3");
	            error_log("keep is $keep");
		            
					$localName = "";
					error_log("sono arrivato3");
	                AJXP_Controller::applyHook("dl.localname", array("ajxp.fs://1/".$file_names[0], &$localName, $this->wrapperClassName));
	                fsAccessDriver::readFile("ajxp.fs://1/".$file_names[0], "force-download", $localName);
	                if ($keep!="true") {
						unlink('/var/www/html/ajp/data/personal/'.$this->user_id.'/'.$file_names[0]);	
					}
	            }
		...
		case "sharewith":
			...
			if (!isset($_SESSION['lastName'])) {
				session_start();
							...
							$con = mysql_connect('flyback.cnaf.infn.it', 'PortalUser', 'PortalUserpassword');
...
//				$result = $result."<Permissions User=\"$permission_values[3]\" Group=\"$permission_values[4]\" Other=\"$permission_values[5]\" />";
			...
			case "ls":
				...
				if ((!isset($_COOKIE['ajxp_wall'])) || ($_COOKIE['ajxp_wall']!="true")) { 	
					...
					$_SESSION['home'] = $home_path;
					error_log("home pathe is $home_path");
				}	
...
*/	
	...
	function download($selectedFiles, &$logMessages, $request) {
		...
		{	
			...
			foreach ($replica_values2 as $element2)
			{
				if($error='true'){
					...
					$command = "sudo X509_USER_PROXY=$this->x509_user_proxy LFC_HOST=$this->lfc_server LCG_GFAL_INFOSYS=$this->lcg_gfal_infosys  lcg-cp --vo $element2  $fs_file";
					
					error_log("command is $command");
					
								
		        	exec("sudo X509_USER_PROXY=$this->x509_user_proxy LFC_HOST=$this->lfc_server LCG_GFAL_INFOSYS=$this->lcg_gfal_infosys  /usr/bin/lcg-cp --vo $vo_active $element2  $fs_file --sendreceive-timeout 10 ", $download_values);	
		        	...
		        	$size2=filesize($downloaded_file_name);
		        		...
		        		error_log("33333333");
			        		exec("sudo X509_USER_PROXY=$this->x509_user_proxy LFC_HOST=$this->lfc_server LCG_GFAL_INFOSYS=$this->lcg_gfal_infosys  /usr/bin/lcg-cp --vo $vo_active $element2  $fs_file ", $download_values);			        
			        		$error=='false';	
		        	    }
		        	}
		       }	
